...
class User:
    ...
    def create_user(cls,data):
        query = """
            INSERT INTO users (first_name,last_name,trainer_name,email,password)
            VALUES (%(first_name)s,%(last_name)s,%(trainer_name)s,%(email)s,%(password)s)
            """
    ...
    @classmethod
    def get_by_trainer_name(cls,data):
            query= """
            SELECT * FROM users WHERE trainer_name = %(trainer_name)s
            """
            results = connectToMySQL(DATABASE).query_db(query,data)
            if results:
                return cls(results[0])
    ...
    def validate_user(data):
        ...
        if len(data['trainer_name']) < 1:
            flash('Please enter a TRAINER NAME', 'reg')
            is_valid = False
        elif len(data['trainer_name']) < 2:
            flash('TRAINER NAME must contain more than 2 characters', 'reg')
            is_valid = False    
        else:
            potential_user = User.get_by_trainer_name({'trainer_name': data['trainer_name']})
            if potential_user:
                flash('This trainer name is already taken', 'reg')
            is_valid = False
        if len(data['email'])< 1:
